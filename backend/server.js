...
// Server-side product catalog. NEVER trust a price sent from the browser â€”
...
app.post('/api/stripe-webhook', express.raw({ type: 'application/json' }), async (req, res) => {
  ...
  if (event.type === 'checkout.session.completed') {
    ...
    // success page (see success.html) â€” EmailJS is a client-side SDK, it
    ...
    // SendGrid, SES) and call it in this block â€” that also works if the
  ...
  try {
    ...
    const session = await stripe.checkout.sessions.create({
      ...
      metadata: {
        ...
        // Stripe metadata values are capped at 500 chars â€” fine for a normal address.
